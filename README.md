...
- the strip also carries the **crew notice board**, read from the Google Doc dispatch keeps it in. it is parsed as a **single** standing alert rather than one per line — every section on it is true at the same time — and it ranks **below NYC Ferry's own alerts and above everything else**: a boat cancelled ten minutes ago outranks a standing notice, and a standing notice outranks a subway closure. headings with nothing under them (`Upcoming:`, `Future Alerts:`) are left off until somebody writes under them. a heading is a line ending in a colon and under 60 characters, which is a heuristic — notices with colons in the middle of them (`Governors Island: ...`) are read correctly, but a short notice that *ends* on a colon would be read as an empty heading and dropped. the document is polled with the other alert sources, so an edit reaches the board within about two minutes; it is never linked from the board, and if it stops being shared Google answers with a sign-in page, which is refused rather than published. see `lib/crew-notices.js`.
