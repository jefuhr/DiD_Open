...
import { fetchCrewNotices } from "./crew-notices.js";
import { fetchSiFerryAlerts, fetchSubwayAlerts, SUBWAY_LOOKAHEAD_MS } from "./transit-alerts.js";
...
function normalizedAlertIsActive(alert, nowMs) {
  ...
  // A ferry alert is only worth showing while it is running. A subway closure is the opposite: it
  // is fetched precisely because it has not started yet — "no [3] overnight" is information at
  // half past eight, and useless at two in the morning when the last boat has gone. So that source
  // is allowed to lead its own start by the same window it was fetched with, and still disappears
  // the moment it ends.
  const lookaheadMs = alert.source === "mta-subway" ? SUBWAY_LOOKAHEAD_MS : 0;
  return periods.some((period) => {
    ...
    return nowMs >= start - lookaheadMs && nowMs <= end;
  });
}
...
export async function fetchServiceAlerts({
  ...
  if (typeof fetchImpl !== "function") throw new Error("A fetch implementation is required.");
  const [gtfsResult, websiteResult, crewResult, subwayResult, siFerryResult] = await Promise.allSettled([
    ...
    fetchCrewNotices({ fetchImpl, now, timeoutMs, contact }),
    fetchSubwayAlerts({ fetchImpl, now, timeoutMs, contact }),
    fetchSiFerryAlerts({ fetchImpl, now, timeoutMs, contact })
  ]);
  // Only the ferry's own sources can make this board's alerts unavailable. The subway and the
  // Staten Island Ferry are context, and context going missing is not the board failing.
  ...
  // One alert or none: the notice board is a single standing notice, and an empty document is not
  // a failure to report — it is dispatch having nothing to say.
  const crewNotices = crewResult.status === "fulfilled" && crewResult.value ? [crewResult.value] : [];
  const subwayAlerts = subwayResult.status === "fulfilled" ? subwayResult.value || [] : [];
  // null rather than an array means the source is dormant: nobody has issued a 511NY key. That is
  // the normal state of this deployment and must not be reported as a failure.
  const siFerryDormant = siFerryResult.status === "fulfilled" && siFerryResult.value === null;
  const siFerryAlerts = siFerryResult.status === "fulfilled" ? siFerryResult.value || [] : [];
  const ferryAlerts = [...(gtfsSnapshot?.alerts || []), ...websiteAlerts]
    .map((alert) => ({ agency: "NYC Ferry", ...alert }));
  return {
    available: true,
    stale: false,
    ...
    sourceStatus: {
      ...
      "crew-notices": crewResult.status === "fulfilled" ? "updated" : "unavailable",
      "mta-subway": subwayResult.status === "fulfilled" ? "updated" : "unavailable",
      "si-ferry": siFerryDormant ? "disabled" : siFerryResult.status === "fulfilled" ? "updated" : "unavailable"
    },
    // NYC Ferry first, always. This is a ferry board: whatever else is on it, the boat comes first,
    // and the bar above the list can only ever show whichever alert leads. The crew notice board
    // sits directly behind them and ahead of everything else: a boat cancelled ten minutes ago
    // outranks a standing notice, and a standing notice outranks a subway closure.
    alerts: [...ferryAlerts, ...crewNotices, ...siFerryAlerts, ...subwayAlerts]
  };
}
