...
export async function fetchServiceAlerts({
  ...
  if (typeof fetchImpl !== "function") throw new Error("A fetch implementation is required.");
  const [gtfsResult, websiteResult, subwayResult, siFerryResult] = await Promise.allSettled([
  ...
  return {
    ...
    // and the bar above the list can only ever show whichever alert leads.
    alerts: [...ferryAlerts, ...siFerryAlerts, ...subwayAlerts]
  };
}
