...
export function createConnectionIndex(byLanding) {
  ...
  return { byStop, stops, routes, tripStops, calendars, exceptions, timezone, serviceCache: new Map() };
}
...
export function nextDepartures({ index, stopId, now = new Date(), limit = DEFAULT_LIMIT, updates = new Map(), vehicles = new Map(), stale = false }) {
  ...
  // service day. Drop this loop and every board loses its first hour after midnight.
  for (let offset = -1; offset <= 0; offset += 1) {
    ...
    for (const departure of index.byStop.get(stopId) || []) {
      ...
      if (delta < -DEPARTED_GRACE_SECONDS) continue;
      ...
      candidates.push({
        ...
        hasLiveTiming,
        // Only a vessel the feed has actually matched to this trip. The board's "McShane?" guess
        // from a boat assignment is deliberately not carried into a secondary panel.
...
export function tripConnections({ index, tripId, limit = DEFAULT_LIMIT, updates = new Map(), vehicles = new Map(), now = new Date(), stale = false }) {
    ...
    stops: [...schedule]
      ...
      .map((call) => {
        ...
        return {
          ...
          connections: nextDepartures({ index, stopId: call.stopId, now, limit, updates, vehicles, stale })
        };
      })
  };
}
