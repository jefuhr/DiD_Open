...
export async function loadSftpOverrideConfig({ configPath, rootPath }) {
  // No config file means the feature is off, not that the board cannot start.
  //
  // config/sftp.json holds a host, a username and the path to a private key, so it is deployment
  // secrets rather than source and is not committed. Reading it unconditionally made a clean
  // checkout refuse to boot at all — the server threw ENOENT before it ever served a departure —
  // while a deployed box carried on working only because it still had an old copy of the file on
  // disk that nothing deletes. A board with no landing notices configured is a board that shows no
  // landing notices, which is what an absent file plainly means.
  //
  // Only "the file is not there" is treated this way. A file that exists and is malformed still
  // throws, because that is someone having configured this and got it wrong, and failing quietly
  // would leave a kiosk silently unable to receive the notices it was set up to show.
  const raw = await readFile(configPath, "utf8").catch((error) => {
    if (error?.code === "ENOENT") return null;
    throw error;
  });
  if (raw === null) return { enabled: false, pollSeconds: 10, readyTimeoutSeconds: 15, verboseErrors: true };
...
export function parseSftpOverrideFile(content, expectedLandingId, landingIds) {
  const expected = normalizeLandingId(expectedLandingId, landingIds);
  let parsed;
  try {
  ...
  const landingId = normalizeLandingId(parsed?.landingId, landingIds);
...
export function createSftpOverridePoller({
  ...
  const landingId = normalizeLandingId(landingIdValue, landingIds);
  ...
  async function performPoll() {
    ...
    try {
      ...
      let remote;
      try {
        remote = parseSftpOverrideFile(content, landingId, landingIds);
      } catch (error) {
